...
from muondecay.detect import detect_queue
from muondecay.analysis import FitResults, data_analysis
...
def main():
	...
	class MuonApp:
		def __init__(self, root, q):
			...
			self.rcanvas = FigureCanvasTkAgg(self.rfig,
				master=self.rateframe)
		...
		def configure(self):
			...
			try:
				...
				port_frame = tk.LabelFrame(self.config_win,
					text="Select " + "Port", foreground="black")
				port_frame.pack()
				for dv in ports:
					dv_rbutton = tk.Radiobutton(port_frame, text=dv,
						foreground="black", variable=self.portname,
						value=dv)
					dv_rbutton.pack(anchor=tk.W)
				outfile_frame = tk.LabelFrame(self.config_win,
					text="Output File", foreground="black")
				outfile_frame.pack()
				outfile_button = tk.Button(outfile_frame,
					text="Select File", foreground="black",
					command=self.outfile_dialog)
				...
				dt_frame = tk.LabelFrame(self.config_win,
					text="Detect Time", foreground="black")
				dt_frame.pack()
				...
				dttext = ['Run until program end', '1 h', '1 d', 
				    '2 d', '7 d']
				dtvals = [0, 3600, 24 * 3600, 2 * 24 * 3600, 
				    7 * 24 * 3600]
		...
		def load_datafile(self):
			...
			filedecays = (filedata[:, 0][filedata[:, 0] < 20000] /
				1000)
			if apnd:
		...
		def pause(self):
			...
			self.paused = True
			self.startbutton.config(text='Resume',
				command=self.resume)
		...
		def update_histogram(self):
			...
			if not self.paused:
				while not self.q.empty():
					...
					logger.debug(f'newdecays: {newdecays}; data: '
						+ f'{self.data}')
					self.ax.clear()
					self.bincounts, _, _ = self.ax.hist(self.data,
						bins=self.bins, edgecolor="black",
						label='Data')
					self.ax.set_title("Muon Decay Times")
					self.ax.set_xlabel(r'Time ($\mu$s)')
					self.ax.set_ylabel('Counts')
		...
		def fit(self):
			...
			columns = ('Parameter', 'Estimate', 'Std Error', 
			    'T Value', 'DOF', 'P(>|T|)')
			...
			self.fit_table = [
				('a', f'{fit.a:.3g}', f'{fit.delta_a:.3g}',
					f'{fit.t_a:.3g}', f'{fit.t_dof:g}',
					f'{fit.p_a:.2g}'),
				('n0', f'{fit.n0:.3g}', f'{fit.delta_n0:.3g}',
					f'{fit.t_n0:.3g}', f'{fit.t_dof:g}',
					f'{fit.p_n0:.3g}'),
				('tau', f'{fit.tau:.4g}', f'{fit.delta_tau:.4g}',
					f'{fit.t_tau:.3g}', f'{fit.t_dof:g}',
					f'{fit.p_tau:.3g}')
			]
			...
			button_frame.pack()
			savefitbutton = tk.Button(button_frame, 
			    text='Save Fit Parameters', 
		...
		def savefig(self):
			if os.path.dirname(self.figfname):
				...
				idir = os.curdir
			self.figfname = filedialog.asksaveasfilename(
			    initialdir=idir,
		...
		def savefit(self, fit):
			if os.path.dirname(self.fitfname):
				...
				idir = os.curdir
			self.fitfname = filedialog.asksaveasfilename(
			    initialdir=idir,
			...
			with open(self.fitfname, 'w') as fitfile:
				...
				np.savetxt(fitfile, line, fmt='%s', delimiter=',')
				line = np.array([['Parameter', 'Estimate', 
				    'Std Error', 'T Value', 'DOF', 'P(>|T|)']],
					dtype='object')                                 
				np.savetxt(fitfile, line, fmt='%s', delimiter=',',
					header='\nFit Parameters')
				line = np.array([["a", f'{fit.a:g}',
					f'{fit.delta_a:g}', f'{fit.t_a:g}',
					f'{fit.t_dof:g}', f'{fit.p_a:g}']],
					dtype='object')
				np.savetxt(fitfile, line, fmt='%s', delimiter=',')
				line = np.array([["n0", f'{fit.n0:g}',
					f'{fit.delta_n0:g}', f'{fit.t_n0:g}',
					f'{fit.t_dof:g}', f'{fit.p_n0:g}']],
					dtype='object')
				np.savetxt(fitfile, line, fmt='%s', delimiter=',')
				line = np.array([["tau", f'{fit.tau:g}',
					f'{fit.delta_tau:g}', f'{fit.t_tau:g}',
					f'{fit.t_dof:g}', f'{fit.p_tau:g}']],
					dtype='object')
				np.savetxt(fitfile, line, fmt='%s', delimiter=',')
