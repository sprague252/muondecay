...
def muon_GUI():
    ...
    def open_window():
        ...
        try:
            ...
            port_frame = tk.LabelFrame(config_win, text="Select Port")
            port_frame.pack()
            for dv in devs:
                dv_rbutton = tk.Radiobutton(port_frame, text=dv, 
                    variable=device, value=dv)
                dv_rbutton.pack(anchor=tk.W)
            outfile_frame = tk.LabelFrame(config_win, text="Output File")
            outfile_frame.pack()
            outfile_button = tk.Button(outfile_frame, text="Select File",
                command = outfile_dialog)
            outfile_button.grid(row=0, column=0)
            outfile_label = tk.Label(outfile_frame, textvariable=outfname)
    ...
    quit_button = tk.Button(control_frame,  text="Quit", width=10, 
        foreground="red", command=master.destroy)
    quit_button.grid(row=5, column=0, sticky=tk.W+tk.E+tk.N)

    monitor_frame = tk.LabelFrame(left_frame, text="Monitor", padx=75,
        pady=5)
    ...
    rate_frame  = tk.LabelFrame(left_frame, text="Rate Meter", padx=5,
        pady=5)
    ...
    detector_frame = tk.LabelFrame(right_frame, text="Muons " + 
        "Through Detector", width=400, height=100, padx=5, pady=5)
